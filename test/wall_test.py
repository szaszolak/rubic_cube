...
class WallTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wall_transposed = Wall(self.initial_state.transpose())
    ...
    def test_get_left_column(self):
        col = np.matrix('1;1;1')
        self.assertTrue(np.array_equal(col, self.wall_transposed.get_left_column()))

    def test_get_right_column(self):
        col = np.matrix('3;3;3')
        self.assertTrue(np.array_equal(col, self.wall_transposed.get_right_column()))
    ...
    def test_swap_left_column(self):
        ...
        returned_col = self.wall_transposed.swap_left_column(inserted_col)
        self.assertTrue(np.array_equal(self.wall_transposed.get_left_column(), inserted_col))
        self.assertTrue(np.array_equal(returned_col, np.matrix('1;1;1')))

    def test_swap_right(self):
        inserted_col = np.matrix('2;2;2')
        returned_col = self.wall_transposed.swap_right_column(inserted_col)
        self.assertTrue(np.array_equal(self.wall_transposed.get_right_column(), inserted_col))
